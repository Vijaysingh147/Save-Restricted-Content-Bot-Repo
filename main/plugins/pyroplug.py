...
async def copy_message_with_chat_id(client, sender, chat_id, message_id):
    ...
    try:
        ...
        caption = f"{final_caption}\n\n__**{custom_caption}**__" if custom_caption else f"{final_caption}\n\n__**[AJ_courses](https://t.me/AJ_courses)**__"
...
async def send_video_with_chat_id(client, sender, path, caption, duration, hi, wi, thumb_path, upm):
    ...
    try:
        gagn = await client.send_video(
            ...
            progress_args=(
                client,
                '**__Uploading: [AJ_courses](https://t.me/AJ_courses)__**\n ',
                upm,
                time.time()
            )
        )
...
async def send_document_with_chat_id(client, sender, path, caption, thumb_path, upm):
    ...
    try:
        gagn = await client.send_document(
            ...
            progress_args=(
                client,
                '**__Uploading:__**\n**__Bot made by [AJ_courses](https://t.me/AJ_courses)__**',
                upm,
                time.time()
            )
        )
...
def get_user_rename_preference(user_id):
    # Retrieve the user's custom renaming tag if set, or default to '@AJ_courses'
    return user_rename_preferences.get(str(user_id), '@AJ_courses')
...
async def get_msg(userbot, client, sender, edit_id, msg_link, i, file_n):
    ...
    if 't.me/c/'  in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            user_session = user_sessions.get(sender)
            if user_session:
              file = await user_bot.download_media(msg, progress=progress_for_pyrogram, progress_args=(client, "**__Unrestricting__: __[AJ_courses](https://t.me/AJ_courses)__**\n ", edit, time.time()))
              await user_bot.stop()
            else:
              file = await userbot.download_media(msg, progress=progress_for_pyrogram, progress_args=(client, "**__Unrestricting__: __[AJ_courses](https://t.me/AJ_courses)__**\n ", edit, time.time()))            # Retrieve user's custom renaming preference if set, default to '@AJ_courses' otherwise
        ...
        try:
           ...
           # medi =  msg.document or msg.video or msg.audio or None
            ...
            file = await userbot.download_media(
                ...
                progress_args=(
                    client,
                    "**__Unrestricting__: __[AJ_courses](https://t.me/AJ_courses)__**\n ",
                    edit,
                    time.time()
                )
            )  
            ...
            if str(file).split(".")[-1] in ['mkv', 'mp4', 'webm', 'mpe4', 'mpeg', 'ts', 'avi', 'flv', 'org']:
                ...
                caption = f"{msg.caption}\n\n__Unrestricted by **[AJ_courses](https://t.me/AJ_courses)**__" if msg.caption else "__Unrestricted by **[AJ_courses](https://t.me/AJ_courses)**__"
                await send_video_with_chat_id(client, sender, path, caption, duration, hi, wi, thumb_path, upm)
            elif str(file).split(".")[-1] in ['jpg', 'jpeg', 'png', 'webp']:
                if file_n != '':
                    #path = ''
